 ...
 */
export function isBitifulDomain(url: string, bitifulDomains: string[]): boolean {
	try {
 ...
 */
export async function getBitifulDimension(imageUrl: string): Promise<{ width: number; height: number } | null> {
	try {
		...
		const data = await response.json() as { ImageWidth: number; ImageHeight: number };
 ...
 */
export async function getBitifulThumbhash(imageUrl: string): Promise<string | null> {
	try {
		const baseUrl = imageUrl.split("?")[0];
